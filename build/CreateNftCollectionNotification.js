const secondSlide = `<div class="topBar">
    <div class="flex flex-row justify-center items-center">
            <p id="accountName" style="margin: 0px 7px 0px 0px;"></p><span title="Account address" id="accountAddress" class="text-lightgray" style="cursor: pointer;"></span></div>
    </div>
    ...
    <div class="main">
        ...
        <div class="middle">
            ...
            <div>
                <div></div>
                <p>Creating a new NFT collection name: <b id="name"></b></p>
            </div>
        </div>
...
function CreateLogicFunction() {
    if (CurrentSlide === 1) {
            ...
            const UseModuleCreateWallet = async () => {
                ...
                try {
                    chrome.storage.local.get(["locked"], async (resp) => {
                        try {
                            const decrypted = await security.loadMnemonic(input.value, resp.locked)
        ...
        const nftCollNameText = document.getElementById("name");
        ...
        nftCollNameText.innerText = NftCollectionName;
